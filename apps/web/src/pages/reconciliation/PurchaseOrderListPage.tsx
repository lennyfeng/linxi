...
const statusColor: Record<string, string> = {
  matched: 'green', partial: 'orange', pending: 'default', not_linked: 'red',
};
...
const PurchaseOrderListPage: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/reconciliation/purchase-orders');
  ...
  const handleSync = async () => {
    setSyncing(true);
    try { await apiClient.post('/reconciliation/sync/purchase-orders', {}); await load(); } catch { /* ignore */ }
    setSyncing(false);
  };

  const filtered = keyword
    ? data.filter((r) => r.orderNo?.toLowerCase().includes(keyword.toLowerCase()) || r.supplierName?.toLowerCase().includes(keyword.toLowerCase()))
    : data;
  ...
  const columns: ColumnsType<PO> = [
    ...
    { title: '发票状态', dataIndex: 'invoiceStatus', key: 'invoiceStatus', width: 140,
      render: (v: string) => <Tag color={statusColor[v] || 'default'}>{v || 'pending'}</Tag> },
    { title: '最近同步', dataIndex: 'sourceUpdatedAt', key: 'sourceUpdatedAt', width: 180,
      render: (v: string) => v || '-' },
  ];
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Typography.Title level={4} style={{ margin: 0 }}>采购单</Typography.Title>
        <Space>
