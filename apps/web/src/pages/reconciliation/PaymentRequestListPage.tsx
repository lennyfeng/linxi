...
interface PR { id: number; requestNo: string; supplierName: string; amount: number; invoiceStatus: string | null; sourceUpdatedAt: string | null; }
...
const PaymentRequestListPage: React.FC = () => {
  ...
  const load = async () => { setLoading(true); try { const res = await apiClient.get('/reconciliation/payment-requests'); setData(res.data?.data ?? []); } catch {} setLoading(false); };
  useEffect(() => { load(); }, []);

  const handleSync = async () => { setSyncing(true); try { await apiClient.post('/reconciliation/sync/payment-requests', {}); await load(); } catch {} setSyncing(false); };

  const filtered = keyword ? data.filter((r) => r.requestNo?.toLowerCase().includes(keyword.toLowerCase()) || r.supplierName?.toLowerCase().includes(keyword.toLowerCase())) : data;

  const columns: ColumnsType<PR> = [
    { title: '请款单号', dataIndex: 'requestNo', key: 'requestNo', width: 180 },
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Typography.Title level={4} style={{ margin: 0 }}>请款单</Typography.Title>
        <Space>
