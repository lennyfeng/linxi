...
const CounterpartyListPage: React.FC = () => {
  ...
  const fetch = useCallback(() => {
    setLoading(true);
    apiClient.get('/ledger/reports/counterparty-breakdown', { params: { startDate: `${year}-01-01`, endDate: `${year}-12-31` } })
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card
        title={<Typography.Title level={4} style={{ margin: 0 }}>Counterparty Management</Typography.Title>}
      ...
      >
        <Table
          ...
          loading={loading}
          columns={[
            { title: 'Counterparty', dataIndex: 'name', render: (v: string) => <Typography.Text strong>{v}</Typography.Text> },
            ...
            { title: 'Count', dataIndex: 'count', width: 80, align: 'right' as const },
            {
              ...
              render: (_: unknown, r: CounterpartyItem) => (
                <a onClick={() => navigate(`/ledger/transactions?counterparty=${encodeURIComponent(r.name)}`)}>
